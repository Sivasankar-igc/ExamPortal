...
const TeacherLP = () => {
    ...
    const login = () => {
        axios.post("http://localhost:8000/teacherLogin", { roll, password })
            .then((res) => {
                if (res.data != "") {
                    let t_info = res.data;
                    nav("/teacherHomePage", { state: { data: t_info } })
                } else {
                    window.alert("Login Failed. Please enter roll and password correctly")
                }
            })
            .catch((err) => console.error(`Teacher Login Error =>>> ${err}`))
    }
